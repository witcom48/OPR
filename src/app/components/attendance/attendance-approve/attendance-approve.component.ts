...
@Component({
  ...
  styleUrls: ['./attendance-approve.component.scss']
})
export class AttendanceApproveComponent implements OnInit {
    ...
    this.menu_projobemp = [
      {
        ...
        command: async (event) => {
          if (this.selectedLostwages.length > 0) {
            this.approveNote = ""
            await this.approve_ATT("W")
  ...
  selectedLostwages: LostwagesModel[] = [];

  selectedLostwagess: LostwagesModel = new LostwagesModel();
  selectedLostwages_name: string = ""
  // selectedstatusProjobemps: LostwagesModel = new LostwagesModel();
  doLoadProjobemp() {
    // this.projectDetailService.projobemp_get(  this.project_code ).then(async (res) => {

    this.lostwagesService.AttApprove_get_withstatus(this.initial_current.CompCode, "", "", "", this.status_select.code).then(async (res) => {
  ...
  doLoadProject2() {
    var tmp = new LostwagesModel();
    tmp.company_code = this.initial_current.CompCode
    tmp.project_code = ""
    tmp.lostwages_status = this.status_select.code

    this.lostwagesService.AttApprove_get_withstatus(this.initial_current.CompCode, this.project_code, "", "", "F").then(async (res) => {
  ...
  async doGetProjobempFillter() {
    const workerfillter: LostwagesModel = new LostwagesModel();
    workerfillter.company_code = this.initial_current.CompCode;
    workerfillter.lostwages_status = this.selectedstatusLostwages;
    console.log(workerfillter.company_code, 'company_code ')

    this.Lostwages_list = await this.lostwagesService.AttApprove_ge_getbyfillter(workerfillter);
    console.log(this.Lostwages_list, 'await uuuuuuuu')
  }
  ...
  //   console.log(workerfillter.lostwages_status,'workerfillter')
  //   this.Lostwages_list = await this.lostwagesService.AttApprove_ge_getbyfillter(workerfillter);
  //   
  ...
  //-- Status สถานะ
  selectedstatusLostwages: string = "";
  ...
  approve_ATT(status: string) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.selectedLostwages.forEach(async (datas: LostwagesModel) => {
          var data = new TRSysApproveModel()
          data.company_code = this.initial_current.CompCode
          data.approve_status = status
          data.project_code = datas.project_code
          data.workflow_type = "ATT_NEW"

          if (datas.worker_code) {
            data.approve_code = datas.worker_code
          } else if (datas.lostwages_cardno) {
            data.approve_code = datas.lostwages_cardno
          }
  ...
  onRowSelectProjobemp(event: Event) {
    this.selectedLostwages_name = this.projobemp_emp + " : " + this.doGetEmployeeDetail(this.projobemp_emp)
  }
}
