...
export class AttendanceImportComponent implements OnInit {
  ...
  attendance: { [key: string]: string } = { EN: "attendance", TH: "การเข้าปฏิบัติงาน" }
  importtime: { [key: string]: string } = { EN: "Importtime", TH: "นำเข้าไฟล์เวลา" }
  importt: { [key: string]: string } = { EN: "Import", TH: "นำเข้าเวลา" }
  setting: { [key: string]: string } = { EN: "Setting", TH: "ตั้งค่า" }
  setimport: { [key: string]: string } = { EN: "Import", TH: "นำเข้า" }
  ...
  doGetInitialCurrent() {
    ...
    this.itemslike = [{ label: this.importtime[this.initial_current.Language], styleClass: 'activelike' },];
