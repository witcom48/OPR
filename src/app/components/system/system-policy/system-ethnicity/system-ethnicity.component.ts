...
export class SystemEthnicityComponent implements OnInit {
  ...
  doDeleteEthnicitys(data: any) {
    this.ethnicityService.ethnicity_delete(data).then((res) => {
      let result = JSON.parse(res);
      if (result.success) {
        this.messageService.add({
          severity: 'success',
  ...
  onRowSelectEthnicity(event: any) {
    this.edit_data = true;
    this.new_data = true;
