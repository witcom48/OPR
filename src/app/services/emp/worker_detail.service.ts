...
export class EmpDetailService {
  ...
  public record_empfamily(worker_code: string, list: EmpFamilyModel[]) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      item_data = item_data + ",\"family_address\":\"" + list[i].family_address + "\"";
      item_data = item_data + "}" + ",";
    }
    if (item_data.length > 2) {
      item_data = item_data.substr(0, item_data.length - 1);
  ...
  public getworker_foreignercard(company: string, code: string) {
    ...
    return this.http.post<any>(this.config.ApiEmployeeModule + '/empforeignercardlist', filter, this.options).toPromise()
      .then((res) => {
        let message = JSON.parse(res);
        // // console.log(res)
        return message.data;
  ...
  public record_empforeignercard(worker_code: string, list: EmpForeignercardModel[]) {
    var item_data: string = "[";
    for (let i = 0; i < list.length; i++) {
      item_data = item_data + "{";
    ...
    return this.http.post<any>(this.config.ApiEmployeeModule + '/empforeignercard', specificData, this.options).toPromise()
      .then((res) => {
        return res;
      });
  }
  public delete_empforeignercard(model: EmpForeignercardModel) {
    const data = {
    ...
    return this.http.post<any>(this.config.ApiEmployeeModule + '/empforeignercard_del', data, this.options).toPromise()
      .then((res) => {
        return res;
      });
  }
  public empforeignercard_import(file: File, file_name: string, file_type: string) {
    const formData = new FormData();
    formData.append('file', file);
  ...
  //Emp Experience
  public getworker_experience(company: string, code: string) {
    var filter = {
      device_name: '',
      ip: "localhost",
      username: this.initial_current.Username,
    ...
    return this.http.post<any>(this.config.ApiEmployeeModule + '/empexperiencelist', filter, this.options).toPromise()
      .then((res) => {
        let message = JSON.parse(res);
        // // console.log(res)
        return message.data;
  ...
  public record_empexperience(worker_code: string, list: EmpExperienceModel[]) {
    var item_data: string = "[";
    for (let i = 0; i < list.length; i++) {
      item_data = item_data + "{";
    ...
    return this.http.post<any>(this.config.ApiEmployeeModule + '/empexperience', specificData, this.options).toPromise()
      .then((res) => {
        return res;
      });
  }
  public delete_empexperience(model: EmpExperienceModel) {
    const data = {
    ...
    return this.http.post<any>(this.config.ApiEmployeeModule + '/empexperience_del', data, this.options).toPromise()
      .then((res) => {
        return res;
      });
  }
  public empexperience_import(file: File, file_name: string, file_type: string) {
    const formData = new FormData();
    formData.append('file', file);
