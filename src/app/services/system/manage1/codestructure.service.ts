...
export class CodestructureService {
  ...
  public getNextCodeEmp(code: string, length_running: Number) {
    // console.log('CODE001..');
    const data = {
      code: code,
      length_running: length_running
    };
    return this.http.post<any>(this.config.ApiSystemModule + '/codenewemp', data, this.options).toPromise()
      .then((res) => {
        let message = JSON.parse(res);
        // console.log(res)
        return message.data;
